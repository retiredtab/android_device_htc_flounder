...
#define LOG_TAG "nv_offload_visualizer"
//#define LOG_NDEBUG 1
#include <assert.h>
#include <math.h>
#include <stdlib.h>
...
void *effects_capture_thread_loop(void *arg __unused)
{
...
int visualizer_hal_start_output(audio_io_handle_t output, int pcm_id) {
    ...
    if (list_empty(&active_outputs_list)) {
        ...
        thread_status = pthread_create(&capture_thread, (const pthread_attr_t *) NULL,
                        effects_capture_thread_loop, NULL);
    }
